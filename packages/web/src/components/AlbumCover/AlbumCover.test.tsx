...
import { componentRenderFactory, ServiceConfig } from '../../test-wrapper';
...
describe('AlbumCover', () => {
  ...
  describe('Should render an shortcode of artist name if imgURL has not been passed', () => {
    ...
    testShortCodeRendered(
      { artistName: 'Ariana Grande', musicTitle: 'One last time' },
      'AG',
    );
  ...
  it('Should render an image if imgURL has been passed', () => {
    const { rootElement } = renderAlbumCover(
      { musicTitle: 'Celebrity', imgURL },
      serviceConfig,
    );
    expect(rootElement).toHaveStyle(`backgroundImage: url(${imgURL})`);
  });
  it('Should render an image if imgURL has been passed even if artisName is passed', () => {
    const { rootElement } = renderAlbumCover(
      { musicTitle: 'Celebrity', imgURL, artistName: 'IU' },
      serviceConfig,
    );
    expect(rootElement).toHaveStyle(`backgroundImage: url(${imgURL})`);
  });
