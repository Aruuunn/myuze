...
export function AlbumCover(props: AlbumCoverProps): ReactElement {
  ...
  const getShortName = (str: string): string => {
    ...
    if (modifiedString.split(' ').length >= 2) return modifiedString.split(' ').slice(0, 2).map((s) => s[0]).join('');
  ...
  return (
    <div className={styles.root}>
      {!imgURL
        ? <span className={styles.innerText}>{typeof artistName === 'undefined' ? getShortName(musicTitle) : getShortName(artistName) }</span>
        : null}
    </div>
  );
}
